 ...
 * 根据日期转时间戳
 */
+ (long)getSecondsFromTime:(NSDate*)time{
...
+ (NSString *)timeAgoWithDate:(NSDate *)date
{
    ...
    if (todayDistanceValue >= 0) {
        return [LXUtilsDate stringWithFromat:@"HH:mm" date:date];
    }else if (yesterdayDistanceValue >= 0){
        return [NSString stringWithFormat:@"昨天 %@",[LXUtilsDate stringWithFromat:@"HH:mm" date:date]];
    }else if (thisYearDistanceValue >= 0){
        return [LXUtilsDate stringWithFromat:@"MM-dd" date:date];
    }else{
        return [LXUtilsDate stringWithFromat:@"yyyy-MM-dd" date:date];
    }
...
+ (NSString *)remainingTimeWithServiceDate:(NSDate *)date endDate:(NSDate *)endDate{
    ...
    if (dateComponents.day > 0) {
        ...
        return FORMAT(@"剩余%ld天",(long)dateComponents.day);
        
    }else if (dateComponents.hour > 0){
        
        return FORMAT(@"剩余%ld小时",(long)dateComponents.hour);
        
    }else if (dateComponents.minute > 0){
        
        return FORMAT(@"剩余%ld分钟",(long)dateComponents.minute);
...
//    NSInteger unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond;
    ...
    if (dateComponents.day>0) {
        [remainingStr appendFormat:@"%ld天",(long)dateComponents.day];
    }
    if (dateComponents.hour>0) {
        [remainingStr appendFormat:@"%ld时",(long)dateComponents.hour];
    }
    if (dateComponents.minute>0) {
        [remainingStr appendFormat:@"%ld分",(long)dateComponents.minute];
    }
    if (dateComponents.second>0) {
        [remainingStr appendFormat:@"%ld秒",(long)dateComponents.second];
    }
    return remainingStr;
}
